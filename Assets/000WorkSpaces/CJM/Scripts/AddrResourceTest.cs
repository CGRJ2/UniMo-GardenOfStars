...
public class AddrResourceTest : MonoBehaviour
{
    ...
    private async Task SO_ClonePattern()
    {
        try
        {
            ...
            var so = await AddressableLoader.LoadAssetAsync<RegionUnlockCondition>(soKey);   // 원본
            _soClone = ScriptableObject.Instantiate(so);           // 런타임 복제본(내 것)
            Debug.Log($"[TEST] SO_Clone title={((RegionUnlockCondition)_soClone).regionName}, value={((RegionUnlockCondition)_soClone).testValue}");
            AddressableLoader.ReleaseAsset<RegionUnlockCondition>(soKey);                    // 원본 해제(복제본은 계속 사용 가능)
